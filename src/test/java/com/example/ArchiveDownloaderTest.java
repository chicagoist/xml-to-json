...
public class ArchiveDownloaderTest {
    ...
    private static final String JSON_OUTPUT_DIR = "src/test/resources" +
            "/json_output";
    ...
    public void testDownloadZip() throws Exception {
        Method method = ArchiveDownloader.class.getDeclaredMethod(
                "downloadZip", String.class, String.class);
        method.setAccessible(true);

        method.invoke(null, ZIP_URL, DOWNLOAD_DIR);

        File downloadedFile = new File(DOWNLOAD_DIR + "/HSCTWXX_I_00054_new" +
                "-schema.zip");
        assertTrue(downloadedFile.exists(), "The ZIP file should be " +
                "downloaded.");
    }
    ...
    public void testUnzipFile() throws Exception {
        ...
        Method method = ArchiveDownloader.class.getDeclaredMethod("unzipFile"
                , String.class, String.class);
        method.setAccessible(true);

        method.invoke(null, DOWNLOAD_DIR + "/HSCTWXX_I_00054_new-schema.zip",
                EXTRACT_DIR);

        File extractedDir = new File(EXTRACT_DIR);
        assertTrue(extractedDir.listFiles().length > 0, "The ZIP file should " +
                "be extracted.");
    }
    ...
    public void testCreateDirectoryIfNotExists() throws Exception {
        Method method = ArchiveDownloader.class.getDeclaredMethod(
                "createDirectoryIfNotExists", String.class);
        method.setAccessible(true);

        String testDirPath = JSON_OUTPUT_DIR;
    ...
    public void testProcessXmlFiles() throws Exception {
        ...
        Method method = ArchiveDownloader.class.getDeclaredMethod(
                "processXmlFiles", String.class, String.class);
        method.setAccessible(true);

        method.invoke(null, EXTRACT_DIR, JSON_OUTPUT_DIR);

        File outputDir = new File(JSON_OUTPUT_DIR);
        assertTrue(outputDir.listFiles().length > 0, "The XML files should be" +
                " processed to JSON.");
    }
